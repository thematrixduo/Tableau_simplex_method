...
void find_r(
  ...
  if ((global_index>0)&&(global_index < length)&&(k_value>0)) {
  ...
  for(int offset = get_local_size(0)/2;offset > 0;offset >>= 1) 
	...
	if (local_index==0) result[local_index]=index[local_index];
	if (local_index==index[0]) 
		result[1]=factor*k_value;
}
