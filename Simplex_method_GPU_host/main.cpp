...
int findR(cl_double* tableau,int t_col,int t_row,int k,int num_col,int &ret,cl_kernel &find_r, cl_context &context, cl_command_queue &queue,cl_mem &t_buff,cl_mem &result_buff,cl_double ceiling,bool &increase);
...
int main(int argc,char* argv[]){ 
	...
	*/
	...
	setKernelArg(find_max,kernel_pivot_2,find_r,copy_kr,pre_update,t_buff,r_buff,result_buff,result_buff_2,col_k_buff,t_col,t_row+1,num_col+1,boundary_buff,num_col+2,boundary_limit_buff,basic_var_buff);
	...
	*/
	...
	//set the kernel argument provided the argument is a pointer or constant
	setKernelArg(find_max,kernel_pivot_2,find_r,copy_kr,pre_update,t_buff,r_buff,result_buff,result_buff_2,col_k_buff,t_col,t_row,num_col+1,boundary_buff,num_col+2,boundary_limit_buff,basic_var_buff);
	/*
...
cl_mem &boundary_limit_buff,cl_mem &basic_var_buff)
{
	...
	ret = clSetKernelArg(find_r, 6, sizeof(cl_int), (void *)&t_row);
	assert(ret==CL_SUCCESS);
