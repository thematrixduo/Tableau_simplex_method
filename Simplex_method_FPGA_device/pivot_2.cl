...
__kernel
void find_max(
            	__global double* restrict buffer,
            	__local double* restrict scratch,
		__local int* restrict index,
            	__const int length,
		__global int* restrict boundary_active,
...
*/
  for(int offset = get_local_size(0)/2;offset > 0;offset >>= 1) 
   ...
   int index_result=index[0];
	//result[local_index]=index[local_index];
...
void find_r(
	    ...
	    __const int t_col,
            ...
            __global int* restrict result,
	    __global int* restrict boundary_active,
	    __global bound* restrict boundary,
	    __global int* restrict basic_var,
	    __const double ceiling,
	    __global double* restrict float_result
	) {
