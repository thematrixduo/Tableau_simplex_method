...
int row_temp_num,cl_mem &t_buff,cl_mem &boundary_buff,cl_mem &result_buff,cl_mem &result_buff_2,cl_mem &basic_var_buff,cl_mem &boundary_limit_buff,cl_mem &float_result_buff,
 int* boundary_active,bound* boundary_original,void *ptr);
...
int main(int argc,char* argv[]){ 
	...
	*/
	...
	void *ptr_bound = NULL;
	posix_memalign (&ptr_bound, AOCL_ALIGNMENT, (num_col+1)*sizeof(bound));
	bound *boundary=(bound*)ptr_bound;
	/*
	...
	*/
	...
	void *ptr_var = NULL;
	posix_memalign (&ptr_var, AOCL_ALIGNMENT, num_constraint*sizeof(int));
	int *var=(int*)ptr_var;
	...
	*/
	...
	createKernel("./pivot_2.aocx","find_max","pivot_2","find_r","copy_kr","pre_update",find_max,kernel_pivot_2,find_r,copy_kr,pre_update,context,device,ret);
	printf("setting kernel arguments\n");
	void *ptr_boundary_active = NULL;
	posix_memalign (&ptr_boundary_active, AOCL_ALIGNMENT,(num_col+num_artificial_var+1)*sizeof(int));
	int *boundary_active=(int*)ptr_boundary_active;
	...
	*/
	...
	posix_memalign (&ptr_boundary_active, AOCL_ALIGNMENT,(num_col+1)*sizeof(int));
	boundary_active=(int*)ptr_boundary_active;
 ...
 int* boundary_active,bound* boundary_original,void *ptr)
{
	...
	*/
	void *ptr_bound = NULL;
	posix_memalign (&ptr_bound, AOCL_ALIGNMENT, (num_col+num_artificial_var+1)*sizeof(bound));
	bound* boundary=(bound*)ptr_bound;
	for (i=0;i<num_col+num_artificial_var+1;i++)
	{
		if (i<num_col+1)
...
int findMax(cl_double* tableau,int t_col,int t_row,int &ret,cl_kernel &find_max, cl_context &context, cl_command_queue &queue,cl_mem &t_buff,cl_mem &result_buff)
{
	...
	cl_int *z = (cl_int*) malloc(t_col*sizeof(cl_int));
	//**Reading the results**
